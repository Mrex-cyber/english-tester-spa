...
export class TestsResourceService {
  ...
  constructor(private client: HttpClient, private userAuthService: UserAuthResourceService) { }
  ...
  public getUserTests(authOptions: ITokenWithEmail): Observable<ITest[]> {
  ...
  public postTestResultWithId(authOptions: ITokenWithEmail, testId: number, answers: IAnswer[]): Observable<number> {
    this.httpOptions.headers = new HttpHeaders({"Accept": "application/json", "Content-Type": "application/json", "Authorization": "Bearer " + authOptions.token});
